...
# --- Sensibilidad y Movimiento player---
...
var current_collectible_target: CreateHallazgo = null
...
func _ready() -> void:
	...
	if world_nodes.size() > 0:
		...
		print("ERROR: No se pudo encontrar el nodo VoxelWorld en el grupo 'voxel_world_group'.")
...
func _unhandled_input(event: InputEvent):
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.is_pressed():
		if is_instance_valid(current_collectible_target):
			current_collectible_target.do_collect()
			return
		
		raycast.force_raycast_update()
		if raycast.is_colliding():
			var collider = raycast.get_collider()
			if collider is VoxelWorld1:
				var collision_point = raycast.get_collision_point()
				var collision_normal = raycast.get_collision_normal()
				
				var pos_in_front = (collision_point + collision_normal * 0.01).floor()
				
				var pos_behind = (collision_point - collision_normal * 0.01).floor()
				
				var air_voxel_pos = pos_in_front
				var solid_voxel_pos = pos_behind
				
				if voxel_world.get_voxel(air_voxel_pos.x, air_voxel_pos.y, air_voxel_pos.z) != VoxelWorld1.AIR:
					# Si el bloque "de en frente" no es aire, significa que el cálculo se invirtió.
					# Esto puede pasar con normales extrañas. Lo corregimos.
					air_voxel_pos = pos_behind
					solid_voxel_pos = pos_in_front
				
				# La posición final a eliminar es siempre la del bloque sólido.
				var voxel_pos_to_remove = solid_voxel_pos
				
				var voxel_x = int(floor(voxel_pos_to_remove.x))
				var voxel_y = int(floor(voxel_pos_to_remove.y))
				var voxel_z = int(floor(voxel_pos_to_remove.z))
				if is_instance_valid(voxel_world):
					print("Intentando excavar en: (%d, %d, %d)" % [voxel_x, voxel_y, voxel_z])
					voxel_world.set_voxel(voxel_x, voxel_y, voxel_z, VoxelWorld1.AIR)
...
func set_collectible_target(target: CreateHallazgo):
	self.current_collectible_target = target
...
func _physics_process(delta: float):
	...
	move_and_slide()
		...
		#var collider = raycast.get_collider()
		#var collision_point = raycast.get_collision_point()
		#print("PUNTO DE COLISION: ", collision_point)
		#print("COLISIONANDO CON: ", collider)
