...
func _init() -> void:
	...
	set_collision_mask_value(2, true) # Permitir que el raycast del jugador (capa 2) lo detecte
	...
	for y in range(CHUNK_SIZE_Y):
		for z in range(CHUNK_SIZE_Z):
			for x in range(CHUNK_SIZE_X):
				...
				var height = noise.get_noise_2d(float(x), float(z)) * (CHUNK_SIZE_Y / 2.0) + (CHUNK_SIZE_Y / 2.0)
				
				var mineral_noise = noise.get_noise_3d(float(x), float(y) * 0.5, float(z))
				...
				if y > height:
					...
					elif mineral_noise < -0.7:
						world_data[index] = RUBY
					else:
						world_data[index] = STONE
	print("Mundo poblado con ruido Perlin.")

func set_voxel(x: int, y: int, z: int, type: int, damage: float = 0.0):
	...
	# 2. Obtener el tipo de vóxel actual y su durabilidad.
	var current_voxel_type = world_data[index]
	var voxel_coord = Vector3(x, y, z)
	
	# Si el daño es 0, significa que estamos estableciendo el tipo de vóxel directamente (ej. poblando el mundo).
	if damage == 0:
		if current_voxel_type == type:
			return # No hacer nada si el tipo ya es el deseado y no hay daño.
		world_data[index] = type
		voxel_health[voxel_coord] = VOXEL_DURABILITY.get(type, 0.0) # Resetear salud
		generate_mesh()
		return
	
	# Si hay daño, procesar la excavación.
	if current_voxel_type == AIR:
		return # No se puede excavar aire.
	
	# Inicializar la salud del vóxel si no existe.
	if not voxel_health.has(voxel_coord):
		voxel_health[voxel_coord] = VOXEL_DURABILITY.get(current_voxel_type, 0.0)
	
	# Aplicar daño.
	voxel_health[voxel_coord] -= damage
	print("Vóxel en (%d, %d, %d) dañado. Salud restante: %.2f" % [x, y, z, voxel_health[voxel_coord]])
	
	# Si la salud llega a 0 o menos, destruir el vóxel.
	if voxel_health[voxel_coord] <= 0:
		var old_voxel_type = current_voxel_type
		world_data[index] = AIR
		voxel_health.erase(voxel_coord) # Eliminar la entrada de salud.
		generate_mesh()
		print("Vóxel en (%d, %d, %d) destruido." % [x, y, z])
		if old_voxel_type != AIR:
			emit_signal("voxel_mined", old_voxel_type)
...
func generate_mesh():
	...
	#var trimesh_shape = mesh_resource.create_trimesh_shape()
	#collision_shape.shape = trimesh_shape
	#var body_rid = get_rid()
	#PhysicsServer3D.body_set_shape(body_rid, 0, collision_shape.shape.get_rid())
...
# Helper para SurfaceTool para añadir una cara con 4 vértices.
#func _add_quad_to_surface(st: SurfaceTool, v1: Vector3, v2: Vector3, v3: Vector3, v4: Vector3):
	#st.add_vertex(v1)
	#st.add_vertex(v2)
	#st.add_vertex(v3)
	#
